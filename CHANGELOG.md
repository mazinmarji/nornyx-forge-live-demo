...
- Claude confinement measured on native Windows, recorded as NOT ESTABLISHED,
  ...
  either: the pin holds `_run_cli` to exactly ONE spawn, so a second one
  written inside it is refused as well -- measured, `os.system` added there
  fails the pin with `'_run_cli' starts more than one process`.
