...
The digest was checked four times before it was trusted: twice by this agent
with two different implementations, once more immediately before the second
extraction, and once by the script itself. That last one was confirmed by
reading the function rather than assuming it from its name — `install_python`
computes the digest over the WHOLE archive and compares it *before*
`zipfile.ZipFile` is opened, so an archive that does not match is never read as
a zip at all.
...
runtime with `start ""` and no stdio redirection — so the grandchild inherits
the captured pipe write handles. `cmd.exe` exits at once; the pipes stay open;
`subprocess.run` waits for an EOF that cannot arrive, times out at 120 seconds,
kills the already-dead `cmd.exe`, and then blocks in its post-kill drain, which
carries no timeout of its own. The 120-second bound does not bound it.

Stated at the strength it was actually established: the causal observation — it
resumed at that termination and at nothing else — is measured. The
handle-inheritance explanation is consistent with every observation and with the
launcher's own text, but the handle table itself was not enumerated.
