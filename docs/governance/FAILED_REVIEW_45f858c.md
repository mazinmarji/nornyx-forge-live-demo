...
| B: H13–H19 have no attack representation in the "authoritative" inventory | FIXED by making the claim honest rather than by inventing attacks. `delegated_to` was one field doing two jobs: H11/H12 point at mutation catalogues, H13–H19 at ordinary test modules, and both read as "re-proved elsewhere". `COVERED_BUT_UNATTACKED` was introduced to name them, so "19 classes" and "35 attacks" could no longer be read as the same ground. **It is EMPTY at this head, and that is the correct state**: all seven now carry an attack or are recorded `OBSOLETE_HISTORICAL_ATTACK`, and the constant's own comment says "PAST TENSE, and now empty". This row said it "now names the seven", which reads -- in a table headed as what is outstanding NOW -- as the fix having been reverted |
