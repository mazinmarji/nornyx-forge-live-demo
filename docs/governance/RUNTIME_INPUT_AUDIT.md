...
| Reviewer trust store path from env | `reviewer_trust.py:109` (`FORGE_REVIEWER_TRUST_STORE`) | **R3.** Same defect as the approver store: `reviewer_store_path()` reads the environment at every `ReviewerTrustStore.load()`. Must resolve once at security-context startup. |
...
| Builder identity from env | `refresh_governance_evidence.py:1598` (`FORGE_BUILDER_IDENTITY`) | **R3.** Independence is derived by comparing an authenticated reviewer against this value, so an attacker who could set it could name a *different* builder and let the real builder's key pass as independent. Must come from the security context, not the ambient environment. |
