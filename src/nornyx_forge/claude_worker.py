...
this module applies the same rule. On the timeout branch it goes one step
further than the sibling does today: a stream that failed to decode is
fingerprinted there too, where the Codex timeout branch records reason and
offset only. Bringing the sibling to the same point is recorded as an open
item under A-025 in docs/requirements/ASSUMPTIONS.md, not claimed here.
