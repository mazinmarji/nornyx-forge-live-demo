...
738-character line, this adapter's line for a 10-character goal as
`_command_line_length` counts it -- so `winerror` alone cannot say which
bound was hit. The
...
#: host through this adapter with a 10-character goal: an existing
#: 333-character `.cmd` shim under a 738-character line, and a 335-character
#: copy of `python.exe` under a 740-character line, both as
#: `_command_line_length` counts them), so the number alone does not say
#: which bound was hit. Duplicated in `codex_worker.py`.
...
#: tests/test_provider_contract.py on Windows. Duplicated in
#: `codex_worker.py`; a test holds the two equal.
...
def _argument_list_too_long(exc: OSError, command: tuple[str, ...]) -> bool:
    ...
    10-character goal, as an over-long invocation of 738 characters -- the
    sub-bound line this adapter built, named as if it were the refused
    quantity. Duplicated in `codex_worker.py`; a test holds the two rules
    equal."""
